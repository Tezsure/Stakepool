...
const getLastCycleStats = async (currentCycle , network ) => {
    try
    {
        const data = {}
        ...
        console.log(totalPoolRewardWon);
        ...
        cycleData.amountByRange.keyMap.forEach(async (element) => {
            ...
            if(low !== high)
            {
                ...
                {
                    let valueByRange = await cycleData.amountByRange.get(element);
                    let lowPrice = referencePrice + (referencePrice * (low/100) / 100);
                    let highPrice = referencePrice + (referencePrice * (high/100) / 100);
                    data.winningPriceRange = {lowPrice , highPrice};
                    data.totalAmountInWinningRange = valueByRange.c[0]
                    let aggregateROIPercent = (data.totalPoolRewardWon / data.totalAmountInWinningRange) * 100;

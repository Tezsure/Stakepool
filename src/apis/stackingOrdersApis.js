...
export const calculateRewards = async (cycle,low,high,stakedAmount,network) => {
    try {
        ...
        let totalRewards = (cycleData.roi["4"].c[0] * cycleDetails.totalAmount ) / cycleData.roi["5"].c[0]
        ...
        else
        {
            ...
            if(lowAmount === highAmount)
            {
                if(low<0 && cycleDetails.endingPrice<lowAmount)
                {
                    cycleData.amountByRange.keyMap.forEach(async (element) => {
                        if(low === element["0"].c[0] * element["0"].s && high === element["1"].c[0] * element["1"].s)
                        {
                            const valueByRange = await cycleData.amountByRange.get(element);
                            console.log(element["0"].c[0] * element["0"].s, element["1"].c[0] * element["1"].s , valueByRange);
                            let rewardWon = (stakedAmount * cycleDetails.totalRewards) / valueByRange;
            ...
            {
                if(lowAmount<= cycleDetails.endingPrice && cycleDetails.endingPrice<highAmount)
                {
                    cycleData.amountByRange.keyMap.forEach(async (element) => {
                        if(low === element["0"].c[0] * element["0"].s && high === element["1"].c[0] * element["1"].s)
                        {
                            const valueByRange = await cycleData.amountByRange.get(element);
                            console.log(element["0"].c[0] * element["0"].s, element["1"].c[0] * element["1"].s , valueByRange);
                            let rewardWon = (stakedAmount * cycleDetails.totalRewards) / (valueByRange.c[0]);
                            conclusion.rewardWon = rewardWon;
                            conclusion.status="Won"
                        }
